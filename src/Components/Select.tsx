...
export default function UnstyledSelectsMultiple({
  ...
  const [value, setValue] = React.useState<string | null>(title);

  React.useEffect(() => {
    if (title !== value) setData(getFilterData(data, member, value));
    else setData(data);
  }, [value]);

  return (
    <>
      <CustomSelect value={value} onChange={setValue} defaultValue={title}>
        <StyledOption value={title}>{title}</StyledOption>
        {list?.map((el, idx) => (
          <StyledOption key={idx} value={el}>
            {el}
          </StyledOption>
        ))}
      </CustomSelect>
